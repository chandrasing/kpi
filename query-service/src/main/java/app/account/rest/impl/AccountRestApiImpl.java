...
public class AccountRestApiImpl implements AccountRestApi {
    ...
    public CompletableFuture<AccountEntity> findById(String id) {
        try {
            Assert.hasLength(id, "Missing commands id");
            return CompletableFuture.supplyAsync(() -> {
                Optional<AccountEntity> entity = repository.findById(id);
                return entity.<AccountEntityNotFoundException>orElseThrow(() -> new AccountEntityNotFoundException(id));
            });
        } catch (AccountEntityNotFoundException | AssertionError | IllegalArgumentException e) {
            throw e;
        }
    }
